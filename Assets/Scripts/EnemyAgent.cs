...
public class EnemyAgent : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        int moveEnemy = myRandom.Next(0, 50);
        if ((player.transform.position.x - enemy.transform.position.x < -1) && moveEnemy == 1)
        {
            moveDirection = -1;
        }
        else if ((player.transform.position.x - enemy.transform.position.x > 1) && moveEnemy == 1)
            moveDirection = 1;
        else if (moveEnemy == 1)
            moveDirection = 0;
